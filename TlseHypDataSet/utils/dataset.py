...
def sat_split_solver(areas: np.ndarray, p_labeled: float, p_test: float) -> np.ndarray:
    """
    ...
    n_labeled = areas[array_solution[0, :] == 1].sum()
    n_unlabeled = areas[array_solution[1, :] == 1].sum()
    n_test = areas[array_solution[2, :] == 1].sum()
